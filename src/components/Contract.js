...
import { AddressInfo, CommonUtility, ContractUtility } from "../common";
...
export function TokenAddress({ address, protocol }) {
  const openTokenAddress = () => {
    const url = ContractUtility.getTokenAddress(protocol, address);
    window.open(url, "_blank");
  };
  return (
    <ContractAddress onClick={openTokenAddress} title={address}>
      {CommonUtility.addressConvertor(address)}
    </ContractAddress>
  );
}

export function TransactionHash({ tx, protocol }) {
  const openTx = () => {
  ...
  return (
    <ContractAddress onClick={openTx} title={tx}>
      {CommonUtility.addressConvertor(tx)}
    </ContractAddress>
  );
}

export function TransactionWithChild({ tx, protocol, children }) {
  const openTx = () => {
    const url = ContractUtility.getTransaction(protocol, tx);
    window.open(url, "_blank");
  };
...
export function TokenId({ tokenId, address, protocol }) {
  const openTokenId = () => {
    const url = ContractUtility.getTokenId(protocol, address, tokenId);
    window.open(url, "_blank");
  };

  return <ContractAddress onClick={openTokenId}>{tokenId}</ContractAddress>;
}

export function AppUsername({ address, protocol, showVerified }) {
  const openUser = () => {
    if (address) {
      // const url = ContractUtility.getTokenAddress(protocol,address);
      window.open(
  ...
  const { text, hasName } = useMemo(() => {
    const hasName = Object.keys(AddressInfo).find(
      (x) => x.toLowerCase() === (address || "").toLowerCase()
    );
    return {
      text: AddressInfo[hasName] || address,
      hasName: !!hasName,
    };
  }, [address]);

  return (
    <ContractAddress className="cursor-pointer" onClick={openUser}>
      {hasName ? (
        <>
          {text}
          {showVerified && (
            <Tooltip title="This is a verified account">
              <svg aria-label="verified-icon" fill="none" viewBox="0 0 30 30">
                <path
                  fill="rgb(32, 129, 226)"
